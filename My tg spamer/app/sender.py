...
async def process_job(db: AsyncSession, job: Job, cyclic=False):
    templates = (await db.execute(select(Template).where(Template.id.in_(json.loads(job.template_ids_blob))))).scalars().all()
    if not templates:
        job.status = "failed"
    ...
    accs = (await db.execute(select(Account).where(Account.id.in_(account_ids), Account.is_authorized==True))).scalars().all()
    if not accs:
        job.status = "failed"
    ...
    try:
        for acc in accs:
            clients[acc.id] = await telethon_manager.ensure_connected(acc.phone, acc.api_id, acc.api_hash)
    ...
    if cyclic and job.is_cyclic and not job.stopped:
